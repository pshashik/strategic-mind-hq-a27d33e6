...
const RISK_KEYWORDS: Array<{ pattern: RegExp; weight: number }> = [
  { pattern: /\b(war|invasion|airstrike|missile|drone|bombard|clash|shelling)\b/i, weight: 3 },
  { pattern: /\b(nuclear|enrichment|warhead|icbm|uranium)\b/i, weight: 3 },
  { pattern: /\b(sanctions?|embargo|tariff|blockade|trade war)\b/i, weight: 2 },
  { pattern: /\b(protest|unrest|coup|riot|insurgent|rebellion)\b/i, weight: 2 },
...
export function generateLocalArticleAnalysis(article: ArticleLike): ArticleAnalysis {
  ...
  const countries =
    country && country !== "Multiple countries" ? [country] : [];
